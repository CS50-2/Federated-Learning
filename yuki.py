...
Original file is located at
    https://colab.research.google.com/drive/1mDRpk3gT6Y2Kg2UbxEEC5qnIxH3Xhkjz
"""
...
def select_clients(client_loaders, use_all_clients=False, num_select=None,
    ...
    if grc:  # 使用 GRC 选择客户端
        ...
        for client_id, client_loader in client_loaders.items():
            ...
            trained_model, h_i = local_train_fedgra_loss(local_model, client_loader, epochs=5, lr=0.001)
...
def save_metric(save_dir, results_labels, experiment_reuslts):
    if not isinstance(experiment_reuslts, (list, pd.Series, np.ndarray)):
        experiment_reuslts = [experiment_reuslts]  # 如果是标量，转换为长度为 1 的列表
    df = pd.DataFrame({results_labels: experiment_reuslts})
...
def main4():
    ...
    axes[0].plot(time_total_1, acc_1, marker='o', label='GRA Only', color=colors['GRA Only'])
    axes[0].fill_between(time_total_1, acc_1, alpha=0.2, color=colors['GRA Only'])

    axes[0].plot(time_total_2, acc_2, marker='^', label='GRA + Freeze 1 Layer', color=colors['Freeze 1'])
    axes[0].fill_between(time_total_2, acc_2, alpha=0.2, color=colors['Freeze 1'])

    axes[0].plot(time_total_3, acc_3, marker='s', label='GRA + Freeze 2 Layers', color=colors['Freeze 2'])
    axes[0].fill_between(time_total_3, acc_3, alpha=0.2, color=colors['Freeze 2'])

    axes[0].plot(time_total_4, acc_4, marker='D', label='GRA + Prune 30%', color=colors['Prune 30'])
    axes[0].fill_between(time_total_4, acc_4, alpha=0.2, color=colors['Prune 30'])

    axes[0].plot(time_total_5, acc_5, marker='v', label='GRA + Prune 60%', color=colors['Prune 60'])
