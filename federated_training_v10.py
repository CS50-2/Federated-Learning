...
def main():
    ...
    for r in range(rounds):
        ...
        csv_data.append([
            ...
            w_loss, w_diff, elapsed_time, total_params_this_round
        ])
...
def main2():
    ...
    for r in range(rounds):
        ...
        csv_data.append([
            r + 1, acc, total_comm,
            ",".join(map(str, selected_clients)), 'NA', 'NA', elapsed_time, total_params_this_round
        ])
...
def main3():
    ...
    for r in range(rounds):
        ...
        csv_data.append([
            ...
            elapsed_time, total_params_this_round  
        ])
...
def main4():
    ...
    # ✅ 修改点：添加用于动态通信判断的 loss 跟踪变量
    prev_loss = None
    comm_threshold = 0.005
    ...
    for r in range(rounds):
        ...
        if prev_loss is None:
            is_comm_round = True  # 第一次必须通信
        else:
            is_comm_round = abs(prev_loss - curr_loss) > comm_threshold
        prev_loss = curr_loss  # 更新 loss

        print(f"📡 是否通信轮: {is_comm_round}")
        ...
        csv_data.append([
            r + 1, acc, total_comm,
            ",".join(map(str, selected_clients)), w_loss, w_diff,
            time.time() - start_time, total_params_this_round,
            int(is_comm_round)
        ])
...
def main5():
    ...
    for r in range(rounds):
        ...
        print(f"📡 是否通信轮: {is_comm_round}（跳过次数: {skip_count}，阈值: {comm_threshold:.5f}）")
...
def main5_pro2():
    ...
    skip_count = 0
    max_skip = 3
    comm_threshold = 0.01  # 只用 loss 控制
    ...
    for r in range(rounds):
        ...
        print(f"📡 是否通信轮: {is_comm_round}（跳过: {skip_count}，AvgLoss: {avg_loss:.4f}, Threshold: {comm_threshold:.5f}）")
