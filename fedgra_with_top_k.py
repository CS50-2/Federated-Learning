# -*- coding: utf-8 -*-
"""FedGRA with Top-k.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/16COk_Ow8o9qPWpfDV8t2Voy5IMt_DCH4
"""
...
def compute_topk_delta(current_weights, previous_weights, topk_ratio=0.1):
    delta = {}
    for k in current_weights:
        delta[k] = current_weights[k] - previous_weights[k]

    # 展平 delta
    flat = torch.cat([v.flatten() for v in delta.values()])
    total_params = flat.numel()
    k = int(total_params * topk_ratio)
    k = max(k, 1)

    # Top-k 索引和值
    _, indices = torch.topk(flat.abs(), k)
    values = flat[indices]
    return indices, values, total_params, list(delta.keys()), [v.shape for v in delta.values()]
...
# Server-side reconstruction of delta
def reconstruct_delta(indices, values, total_params, keys, shapes):
    flat = torch.zeros(total_params)
    flat[indices] = values

    # Reduced to per-layer parameter format
    pointer = 0
    tensors = []
    for shape in shapes:
        num = torch.prod(torch.tensor(shape)).item()
        tensors.append(flat[pointer:pointer+num].view(shape))
        pointer += num
    return {k: v for k, v in zip(keys, tensors)}

# Aggregate delta update global model
def apply_delta_to_model(model, avg_delta):
    state_dict = model.state_dict()
    for k in state_dict:
        state_dict[k] += avg_delta[k]
...
def main():
    ...
    grc = True
    previous_weights = global_model.state_dict()
    local_epoch = 1
    lr = 0.01
    ...
    for r in range(rounds):
        ...
        for client_id in selected_clients:
            ...
            local_model = MLPModel()
            local_model.load_state_dict(previous_weights)  # 复制全局模型参数
            ...
            idx, val, total_params, keys, shapes = compute_topk_delta(local_state, previous_weights, 0.1)
            indices_list.append(idx)
            values_list.append(val)
            deltas.append((idx, val))
        ...
        # 统计通信成本
        total_comm_cost = []
        comm_cost = sum([2 * len(v) for v in values_list])  # index + value
        total_comm_cost.append(comm_cost)
        print(f"Round {r}: Communication cost = {comm_cost}")

        # 聚合 delta
        avg_flat = torch.zeros(total_params)
        total_data = sum(client_data_sizes)

        for (idx, val), data_size in zip(deltas, client_data_sizes):
            temp = torch.zeros(total_params)
            temp[idx] = val
            avg_flat += temp * (data_size / total_data)  # 加权累加
        ...
        for shape in shapes:
            num = torch.prod(torch.tensor(shape)).item()
            tensors.append(avg_flat[pointer:pointer+num].view(shape))
            pointer += num
        avg_delta = {k: v for k, v in zip(keys, tensors)}

        # 聚合模型参数
        # global_model = fed_avg(global_model, client_state_dicts, client_data_sizes)

        # 更新全局模型
        apply_delta_to_model(global_model, avg_delta)
        previous_weights = global_model.state_dict().copy()
