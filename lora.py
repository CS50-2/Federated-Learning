...
class MLPModel(nn.Module):
    def __init__(self, is_LoRA=False, rank=4, alpha=8):
        super(MLPModel, self).__init__()

        self.is_LoRA = is_LoRA  # Set the LoRA flag
        ...
        if not self.is_LoRA:  # Use the original linear layers
            ...
            self.fc2 = LinearWithLoRA(nn.Linear(200, 200), rank, alpha)
...
def run_experiment(selection_method, rounds=100, num_selected_clients=2, is_LoRA=False):
    torch.manual_seed(0)
    random.seed(0)
    np.random.seed(0)
    ...
    global_model = MLPModel(is_LoRA=is_LoRA)  # Use LoRA if is_LoRA is True
    global_accuracies = []
    ...
    for r in range(rounds):
        ...
        for client_id in selected_clients:
            ...
            local_model = MLPModel(is_LoRA=is_LoRA) # Create local model with LoRA if is_LoRA is True
...
def main_experiments():
    ...
    df_combined = df_high_loss.merge(df_lora, on='Round')
