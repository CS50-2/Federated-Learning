...
def select_clients(client_loaders, use_all_clients=False, num_select=None,
                   select_by_loss=False, global_model=None, grc=False,
                   fairness_tracker=None, lora_rank=4, lora_alpha=1, lr=None):
    if grc:  # 使用 GRC 选择客户端
        client_lora_models = []
        ...
        for client_id, client_loader in client_loaders.items():
            ...
            trained_lora_model, h_i = local_train_fedgra_loss_lora(
                global_model, client_loader, epochs=5, lr=lr,
...
def main(lora_rank=None, lr=None):
    ...
    # 实验数据存储 CSV
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    csv_filename = f"training_data_lora_{timestamp}_{lora_rank}_{lr}.csv"
    csv_data = []
...
if __name__ == "__main__":
    ...
    #         print(f"\n🚀 Running experiment with LoRA Rank = {rank}, Learning Rate = {lr}")
    #         main(lora_rank=rank, lr=lr)
    # T2 = time.time()

    T1 = time.time()
    main(lora_rank=4, lr=0.001)
    T2 = time.time()
    print('程序运行时间:%s秒' % ((T2 - T1)))
    # 1520.884345293045
